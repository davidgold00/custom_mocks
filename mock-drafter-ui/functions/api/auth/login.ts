import {
  type Env, json, now, hashPassword, safeEqual, createSession, sessionCookie,
} from '../../_lib/auth'
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  return json({ user: { id: row.id, username: row.username } }, 200, { 'Set-Cookie': sessionCookie(token, expires) })
}
